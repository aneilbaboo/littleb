...
;;; $Id: species-type.lisp,v 1.55 2008/09/02 14:58:10 amallavarapu Exp $
;;; $Name:  $
...
(define-macro defmonomer (name-lclass &body def)
  `(define-complex-element ,name-lclass monomer ,@def))

(defmacro define-complex-element (name-lclass class &body def)
...
(defun make-complex-graph (&optional descr force-pattern)
  ...
  (handler-case
    ...
    (error (e) (error "Invalid complex: ~A. ~A"
                      (or (ignore-errors (with-output-to-string (stream) 
...
(defun parse-named-monomer-description (monomer-object
  ...
  (let ((sites              (map 'list #'identity (monomer-sites monomer-object)))
        (pattern-detected-p nil)
  ...
  (let ((first (first args)))
    (cond
     ((or (monomer-symbol-ref-p first)
       ...
       `(make-complex ',(expand-user-complex (list args))))
     ((and (object-form-p first)
           (let ((op (object-form-object first)))
             (or (monomer-symbol-ref-p op)
                 (user-defined-complex-function op))))
       `(make-complex ',(expand-user-complex (mapcar #'object-form-body args)))))))
...
(define-macro defcomplex (name slambda-list &body body)
  (let ((sbindings '#:sbindings))
    `(eval-when (:compile-toplevel :load-toplevel :execute)
       (setf (gethash ',name *complex-expansions*) 
             (lambda (,sbindings) (process-complex-macro-output 
                                   ',name 
                                   (sites-destructuring-bind ,slambda-list ,sbindings
                                     ,@body))))
     ',name)))
...
(defun process-site-param  (sparam) 
  (cond
   ;; lambda-list param is X.Y
   ((fld-form-p sparam ) (list (fld-form-object sparam)
                              (fld-form-field sparam)))
   ;; lambda-list param is (X Y)
   ((consp sparam )      sparam)
   
   ;; default - assume X represents a site; default = unbound
   (t                    (list sparam :_))))

(defun find-and-remove (item list &key key test)
  (loop with key = (or key #'identity)
        with test = (or test #'eql)
        with found? = nil
        for rest on list
        for i = (first rest)
        if (funcall test item (funcall key i))
        append (rest rest) into ret
        and do (setf found? t) (loop-finish)
        else collect i into ret
        finally (return (values ret found?))))
        
(defun process-site-bindings (params bindings)
  (loop for b in bindings
        while params
        if (fld-form-p b)
        ;; if it's a fld form, remove the correspond site from params
        collect  (let+ ((site-name (fld-form-object b))
                        (site-binding (fld-form-field b))
                       ((new-params found?) (find-and-remove site-name params :key #'car)))
                  (unless found? 
                    (if params (error "Cannot resolve site binding ~S - expecting one of ~S"
                                      site-name (mapcar #'car params))
                      (error "Too many sites: ~S" bindings)))
                  (setf params new-params)
                  (list site-name site-binding))
        into computed-bindings
        ;; otherwise, we're doing by order:
        else collect (list (car (pop params)) b) into computed-bindings
        finally (return (append computed-bindings params))))

(defmacro sites-destructuring-bind (slambda-list site-bindings &body body)
  (let* ((params    (mapcar #'process-site-param  slambda-list))
         (bindings  '#:bindings))
    `(let ((,bindings (process-site-bindings ',params ,site-bindings)))
       (progv
